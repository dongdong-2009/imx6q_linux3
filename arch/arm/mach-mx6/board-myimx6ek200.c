...
#include "board-myimx6ek200-6q.h"
#include "board-myimx6ek200-6u.h"
...
/*
* spi_board_info用来初始化spi_device
*/
static struct spi_board_info imx6_sabresd_spi_device[] __initdata = {
	{
		.modalias = "spidev",                                                          // 
		.max_speed_hz = 20000000, /* max spi clock (SCK) speed in HZ */
		.bus_num = 1,                                            // 用到的spi主机控制器序号
		.chip_select = 0,                                     // 使用哪一号片选
		.mode = SPI_MODE_0,
	},
};
...
/*
* static void __init mx6_sabresd_board_init(void) 函数调用 spi_device_init   \
												spi2_device_init
*/

/*
在机器的 init_machine() 函数中，注册
*/
...
/*	ret = gpio_request(SABRESD_PFUZE_INT, "pFUZE-int");
	...
	imx6q_add_ecspi(1, &mx6q_sabresd_spi2_data);     // 初始化片选
	spi_device_init();
	spi2_device_init();                                          // spi_register_board_info   注册
...
*/
	...
	/////////////////////////////////add @ jiayu///////////////////////////////////
	/////////////////////  测试过程中输出0/////////////////////////////////////////
	gpio_direction_output(SABRESD_SD2_DAT0_DIR, 1);
	gpio_direction_output(SABRESD_SD2_DAT1_DIR,1);
	gpio_direction_output(SABRESD_SD2_DAT2_DIR,1);
